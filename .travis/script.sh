...
then
	docker run -t -v "${PWD}:/libfcache" ${DOCKERHUB_REPO}:${DOCKERHUB_TAG} sh -c "cd libfcache && .travis/script_docker.sh";
...
then
	...
	then
		...
		cp AUTHORS COPYING NEWS README ${PWD}/osx-pkg/usr/share/doc/libfcache;
