...
LIBRARY_TESTS="cache cache_value date_time error support";
...
run_test()
{
	...
	local TEST_EXECUTABLE="./fcache_test_${TEST_NAME}";
...
run_test_with_input()
{
	...
	run_test_on_input_directory "libfcache" "${TEST_DESCRIPTION}" "default" "${OPTION_SETS}" "${TEST_EXECUTABLE}" "input" "${INPUT_GLOB}";
	local RESULT=$?;
